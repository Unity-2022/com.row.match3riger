...
public class Item : MonoBehaviour, IMatchGameItem
{
    ...
    public static IMatchGameItem Instant(Vector2 target, Vector2 start, Vector2Int _index, int id)
    {
        var _sprite = Resources.Load<Sprite>($"icons/{id}");
        var _item = Instantiate(Resources.Load<Item>("item"));

        _item.GetComponent<SpriteRenderer>().sprite = _sprite;

        _item.transform.parent = GameObject.Find("grid").transform;
